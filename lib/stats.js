module.exports = class Stats
{
    ...
    {   
        ...
        for(let intv_index = start; intv_index < end; intv_index++)
        {
            ...
            if(actual_t_bucket.start <= timestamp && timestamp < actual_t_bucket.start + polling_interval.bucket_size)
            {
                ...
                {
                    actual_t_bucket.sub[sub_index].value.push(value);
            ...
            else if(actual_t_bucket.start + polling_interval.bucket_size <= timestamp)
            {
                ...
                {
                    start: actual_t_bucket.start + polling_interval.bucket_size,
                    min: Infinity,
                    max: -Infinity,
                    cnt: 0,
                    sum: 0,
    ...
    mdn( interval )
    {
        for(let intv_index = 0; intv_index < this.intervals_cnt; intv_index++)
        {
            if(this.intervals[intv_index].interval == interval)
            {
                ...
                for(let i = 0; i < this.intervals[intv_index].precision; i++)
                {   
                    if(this.intervals[intv_index].buckets[i].cnt == 0){continue;}
                    let med_pos = Math.floor( this.intervals[intv_index].buckets[i].cnt / 2 );
                    for(let value_bucket of this.intervals[intv_index].buckets[i].sub )
                    {
                        if(value_bucket === undefined){continue;}
                        med_pos -= value_bucket.cnt;
                        if(med_pos < 0)
                        {
                            value_bucket.value.sort( (a,b) => a - b );
                            medians.push( value_bucket.value[ Math.floor( value_bucket.cnt / 2 ) ] );
                        }
                    }
                }
    ...
    p_interval( interval, p_value)
    {
        let p_num = Math.floor( p_value * this.cnt( interval ) );
        for(let intv_index = 0; intv_index < this.intervals_cnt; intv_index++)
        {
            if(this.intervals[intv_index].interval == interval)
            {
                let first_p = new Array(2 * p_num).fill(this.max(interval));
                for(let i = 0; i < this.intervals[intv_index].precision; i++)
                {
                    let temp = new Array();
                    let value_index = 0;
                    while( temp.length < p_num && temp.length < this.intervals[intv_index].buckets[i].cnt)
                    {
                        if(this.intervals[intv_index].buckets[i].sub[value_index] === undefined){value_index++;continue;}
                        temp = temp.concat(this.intervals[intv_index].buckets[i].sub[value_index].value);
                        value_index++;
                    }
                    first_p.splice(p_num,p_num,...temp);
                    first_p.sort( (a,b) => a - b);
                }
                return first_p[p_num] / this.min( interval );
            }
        }
    }
}
